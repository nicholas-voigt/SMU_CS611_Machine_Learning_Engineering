...
with DAG(
    ...
    ### Customer Financials Processing
    ...
    ### Gold Feature Store Processing

    # join silver feature data to gold feature store & engineer gold features for ml modeling
    gold_feature_store = BashOperator(
        task_id='run_gold_feature_store',
        bash_command=(
            'cd /opt/airflow/scripts/data_processing && '
            'python gold_processing_features.py --date "{{ ds }}"'
        ),
    )
    ...
    dep_check_source_clickstream_data >> bronze_clickstream_store >> silver_clickstream_store >> gold_feature_store # type: ignore
    dep_check_source_customer_attributes_data >> bronze_customer_attributes_store >> silver_customer_attributes_store >> gold_feature_store # type: ignore
    dep_check_source_customer_financial_data >> bronze_customer_financials_store >> silver_customer_financials_store >> gold_feature_store # type: ignore
    gold_feature_store >> feature_store_completed # type: ignore